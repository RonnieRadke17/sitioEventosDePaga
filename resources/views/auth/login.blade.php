...
<form method="POST" action="{{ route('signin') }}" class="max-w-md mx-auto p-6 bg-white shadow-md rounded-lg">
    ...
    <div class="mb-4">
        ...
        <input type="password" name="password" id="password" autocomplete="off" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
        @error('password')
            <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
