                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50 cursor-pointer activity-row" data-activity-id="{{ $activity->id }}">
                        ...
                        <td class="py-2 px-4 text-center">
                            ...
                            {{ in_array($activity->id, old('selected_activities', isset($eventActivities) ? $eventActivities->keys()->toArray() : [])) ? 'checked' : '' }}>
                        </td>
                    </tr>
                    <tr class="hidden activity-details" id="activity-{{ $activity->id }}-details">
                        <td colspan="2" class="py-2 px-4">
                            ...
                            <div class="mb-2">
                                <label class="block font-semibold">
                                    <input type="checkbox" name="genders[{{ $activity->id }}][{{ $gender }}]" value="{{ $gender }}"
                                    {{ in_array($gender, old('genders.'.$activity->id, isset($eventActivities[$activity->id]) ? $eventActivities[$activity->id]->pluck('gender')->toArray() : [])) ? 'checked' : '' }}> {{ $gender }}
                                </label>
                                <div class="pl-4 hidden gender-subs" id="activity-{{ $activity->id }}-gender-{{ $gender }}-subs">
                                    @foreach ($subs as $sub)
                                    <label class="block">
                                        <input type="checkbox" name="subs[{{ $activity->id }}][{{ $gender }}][]" value="{{ $sub->id }}"
                                        @if(in_array($sub->id, old('subs.'.$activity->id.'.'.$gender, isset($eventActivities[$activity->id]) ? $eventActivities[$activity->id]->where('gender', $gender)->pluck('sub_id')->toArray() : [])))
                                            checked
                                        @endif> {{ $sub->name }}
                                    </label>
                                    @endforeach
